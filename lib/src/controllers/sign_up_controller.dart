...
import 'package:promarket/src/components/success_alert.dart';
import 'package:promarket/src/controllers/auth_controller.dart';
import 'package:promarket/src/models/user.dart' as UserModel;
import 'package:promarket/src/requests/reset_password_request.dart';
import 'package:promarket/src/requests/sign_up_request.dart';
...
class SignUpController extends GetxController {
  ...
  Future<void> resend() async {
    await FirebaseAuth.instance.verifyPhoneNumber(
      ...
      codeSent: (String vId, int? resendToken) {
        verificationId.value = vId;
        Get.bottomSheet(SuccessAlert(
          message: "Successfully sent sms".tr,
          onClose: () {
            Get.back();
          },
        ));
  ...
  Future<void> saveUserPassword() async {
    loading.value = true;
    ...
    if (password.value.length < 6) {
      ...
      Map<String, dynamic> data =
          await passwordResetRequest(phone.value, password.value);
      if (data['success'] != null) {
        if (!data['success']) {
          Get.bottomSheet(ErrorAlert(
          ...
          Get.back();
          Get.bottomSheet(SuccessAlert(
            message: "Successfully changed".tr,
            onClose: () {
              Get.back();
            },
          ));
